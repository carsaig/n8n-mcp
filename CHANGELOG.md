...
- **`n8n_manage_credentials` now explains itself when an n8n instance cannot read credentials (#809).** Not every n8n version exposes credential reads in its public API: older instances reject `GET /credentials` outright, so the `list` action failed with a bare `GET method not allowed` and agents could not tell an unsupported action from a transient error. `list` and `get` now detect the rejection and return a clear `NOT_SUPPORTED` response explaining that the instance only supports `create`, `update`, `delete`, and `getSchema`, with a hint to find credential IDs in the n8n UI. The tool description documents the version requirement. Instances whose API does expose credential reads (used by `list` pagination and the `get` list-fallback since 2.57.1) are unaffected.
